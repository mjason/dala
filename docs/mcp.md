...
- **MCP 无法自管。** 管理 MCP 开关与令牌的三个 rpc 动作（`mcp_settings` /
  `set_mcp_enabled` / `regenerate_mcp_token`）**只暴露给 Web UI**，被显式排除在 MCP
...
3. 令牌就显示在面板上：**一键复制**粘贴进你的 MCP 客户端配置（见下一节），
...
- Web UI 用的 rpc 动作：`mcp_settings` / `set_mcp_enabled` / `regenerate_mcp_token`

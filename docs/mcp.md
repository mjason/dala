...
- **输出和附件有硬边界。** `wait_terminal` 是最多 25 秒的事件驱动长轮询，每会话最多
  8 个、全局最多 128 个等待者。附件单个最多 5 MB、私有保存 24 小时，并有速率和
  总容量限制。
...
| `read_terminal` | 从 holder 网格读取纯文本；普通缓冲区含 scrollback，TUI 返回当前 alternate screen。 |
...
| `terminal_upload_attachment` | 上传单个文件/图片到私有的 24 小时受管目录，返回绝对路径。 |

...
| `send_terminal_keys` | 向 TUI 顺序发送最多 100 个安全按键；支持方向键、Home/End、PageUp/PageDown、Space、Tab/Shift-Tab、Enter/Esc 和有限控制键。 |
...
→ send_terminal_keys {keys: ["DOWN", "SPACE", "ENTER"]}
→ wait_terminal（使用返回的 seq）
→ 再次 read_terminal 验证
```

这能覆盖键盘可操作的 TUI 选择。当前没有开放任意鼠标坐标或原始字节注入；只接受工具
schema 中列出的安全按键，避免 Agent 因转义序列错误破坏会话状态。旧 holder 会继续

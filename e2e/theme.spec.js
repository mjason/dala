...
test.describe("Given 一个打开了终端会话的用户，在设置里切换主题", () => {
  ...
  test("切到「亮」：外壳与终端一起变亮；切回「暗」：又一起变暗", async ({ page }) => {
    ...
    await expect.poll(() => cssToken(page, "--color-git-deleted")).toBe("#8f3f5f");
    await expect.poll(() => cssToken(page, "--color-git-conflict")).toBe("#6639ba");
    await expect.poll(() => cssToken(page, "--color-git-ignored")).toBe("#68727c");
    ...
    await expect.poll(() => cssToken(page, "--color-git-deleted")).toBe("#d27d9f");

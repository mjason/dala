...
test.describe("Given 一个带 dala.jsonc 的项目会话，用户打开设置面板", () => {
  ...
  test("桌面：设置面板四个 tab 单行并列", async ({ page }) => {
    // 显式桌面视口：手机上是 2×2 两行（见下面的手机用例），这条断言只对
    // sm 及以上成立。
    await page.setViewportSize({ width: 1000, height: 700 });
    await h.openSettings(page);
    ...
    // 四个不同的 left —— 真的是并排 4 列，而不是叠在一起。
    expect(new Set(rects.map((r) => Math.round(r.left))).size).toBe(4);
  });

  test("外观页首项可切换系统、浅色和深色主题，并持久化显式选择", async ({ page }) => {
    await h.openSettings(page);
    await h.openSettingsTab(page, "appearance");

    const system = page.locator("#theme-system-button");
    const light = page.locator("#theme-light-button");
    const dark = page.locator("#theme-dark-button");
    ...
    expect(await selector.evaluate((element) => element.parentElement?.firstElementChild === element))
      .toBe(true);
    ...
    await expect
      .poll(() => page.evaluate(() => localStorage.getItem("phx:theme")))
      .toBe("light");
    ...
    await page.locator("#theme-system-button").click();
    await expect(page.locator("#theme-system-button")).toHaveAttribute("aria-pressed", "true");
    ...
    await expect
      ...
      .toBe(await page.locator("html").getAttribute("data-theme"));
    await expect
      .poll(() => page.evaluate(() => localStorage.getItem("phx:theme")))
      .toBe(null);
  ...
  test("tab 有完整的 ARIA 语义：左右方向键在 tab 间移动焦点与选中，tabpanel 关联到当前 tab", async ({
    ...
    const last = page.locator('[data-settings-tab="voice"]');
    ...
    await expect(page.locator("#speech-mic-select")).toBeVisible();
...
test.describe("Given 手机上打开设置面板的用户", () => {
  ...
  for (const locale of ["zh-CN", "de"]) {
    test(`${locale}：390 与 320 视口下四个 tab 全在屏内、不被裁，弹窗不越界，语音 tab 可点`, async ({
      page,
    }) => {
      ...
      for (const width of [390, 320]) {
        ...
        expect(geom.tabs).toHaveLength(4);

        // 窄屏是 2×2：两行、每行两个。headless chromium 的 CJK 字形比真机
        // （PingFang/SF）窄半个像素 —— 中文在 4 列下"刚好不裁"，真机上就裁了
        // （用户截图）。所以除了裁切断言，这里直接把 2×2 的版式钉死：任何人
        // 把窄屏改回 grid-cols-4 立刻红。
        const rows = [...new Set(geom.tabs.map((t) => Math.round(t.top)))].sort((a, b) => a - b);
        expect(rows, `${locale}@${width} rows`).toHaveLength(2);
        for (const row of rows) {
          expect(geom.tabs.filter((t) => Math.round(t.top) === row)).toHaveLength(2);
        }
